...
"There are no tables" has truth-conditions that do not require tables: it is true if the domain contains no tables, false otherwise. Evaluation does not require tables. The sentence is evaluable. "There are no evaluators anywhere" also has truth-conditions that do not require evaluators: it is true if the domain contains no evaluators, false otherwise. An evaluator checking it is a counterexample, not a precondition failure — the sentence is simply false. The act of checking requires an evaluator, but the sentence's truth-conditions do not. It is executable. It returns false.
