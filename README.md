...
"This statement is false" — discrimination cannot resolve because the sentence provides no verdict-stabilizing target. It makes the verdict depend only on itself with no independent anchor, so the operation cannot reach a determinate exit condition. {P, ¬P} — the non-executability claim is not that the conjunction is false. It is that a specification demanding incompatible verdicts cannot serve as input to the discrimination operation, because it cancels the very role of a verdict. Discrimination is asked to return both "holds" and "fails to hold" on the same target. It does not return "false." It does not return at all. Classical logic's explosion — from a contradiction, anything follows — is the formal system's representation of this: when discrimination collapses on an input, the consequence relation trivializes, which is the formal signature of "no discriminative resolution possible here."

In each case, the expression is syntactically well-formed — it passes the grammar — but discrimination cannot return a verdict because the expression negates a condition constitutive of discrimination's applicability to that content.
...
Non-executable is not undecidable. An undecidable problem is one no algorithm can settle in general, but each instance still has a determinate answer that evaluation could in principle reach. A non-executable expression is one where evaluation cannot get purchase at all without importing what the expression denies. The limitation is not computational. It is structural. In classical consequence, trivialization — from a contradiction, anything follows — is the formal signature that a contradictory specification has destroyed discriminative power. That is exactly what "no evaluative resolution" means in this framework: when discrimination collapses on an input, the consequence relation trivializes.
