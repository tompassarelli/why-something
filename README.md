...
5. An expression is non-executable if under its intended reading it denies a condition required for discrimination to resolve on that expression. It parses. Discrimination cannot return a verdict.

6. "Nothing exists" and "this statement is false" are two instances of one pattern: expressible, not executable. Both deny a condition required for discrimination to resolve — the first denies the discriminative substrate itself, the second provides no verdict-stabilizing target.
...
The criterion is general: an expression e is non-executable if under its intended reading, e denies a condition required for discrimination to resolve on e. "Required" means constitutive of discrimination's applicability to that very expression: removing the condition does not make the expression false — it removes the possibility of a verdict on that content at all. This is a criterion about what the expression requires, not about who is checking. The question is not whether the act of checking e requires something e denies — almost any globally quantified denial would meet that test, since someone is always doing the checking. The question is whether discrimination can return a verdict on e without presupposing what e negates.
...
"Nothing exists" on the absolute reading is different. Discrimination cannot resolve because the expression denies the conditions under which discrimination operates: structure, distinction, anything to discriminate. There is nothing for discrimination to get purchase on. The expression does not return a negative verdict. It fails to reach the discrimination operation at all. This is what makes it non-executable rather than merely false.
...
{P, ¬P} is a related but distinct case. Read as a single compound claim — "both P and not-P" — it is executable: discrimination returns a negative verdict. The conjunction fails to hold. Read as a constraint set — "require P to hold and require ¬P to hold simultaneously" — it is an inconsistent specification that trivializes inference under classical rules (explosion). But trivialization of inference is a different failure mode from non-executability. Discrimination can return a verdict on the conjunction (it fails). What it cannot do is use the set as a consistent basis for further inference. That problem — inconsistent premise sets and what to do with them — is addressed separately in Passarelli (2026). Here, the relevant non-executable pattern has two instances: "nothing exists" and "this statement is false." Both deny a condition required for discrimination to resolve. {P, ¬P} denies a condition required for inference to proceed, which is a downstream phenomenon.

In both non-executable cases, the expression is syntactically well-formed — it passes the grammar — but discrimination cannot return a verdict because the expression negates a condition constitutive of discrimination's applicability to that content.

"Denies" is precise throughout: under the intended reading of e, the expression asserts or entails the negation of a condition required for discrimination to resolve on e. "There are no tables" does not deny such a condition. "Nothing exists" on the absolute reading does. The criterion does not overgenerate because it operates at the level of discriminative resolution, not at the level of who happens to be checking.
