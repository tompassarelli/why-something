...
The criterion is general: an expression e is non-executable if under its intended reading, e denies a condition required for discrimination to resolve on e. This is a criterion about what the expression requires, not about who is checking. The question is not whether the act of checking e requires something e denies — almost any globally quantified denial would meet that test, since someone is always doing the checking. The question is whether discrimination can return a verdict on e without presupposing what e negates.

"There are no tables" — discrimination resolves. The expression denies tables, not the conditions of discrimination. It is true if the domain contains no tables, false otherwise. Evaluable. "There are no living things anywhere" — discrimination resolves. It is true if the domain contains no living things, false otherwise. A living thing checking it is a counterexample — the expression is simply false. The act of checking requires a living thing, but discrimination on the expression does not. Executable. Returns false.

"Nothing exists" on the absolute reading is different. Discrimination cannot resolve because the expression denies the conditions under which discrimination operates: structure, distinction, domain. There is nothing for discrimination to get purchase on. The expression does not return a negative verdict. It fails to reach the discrimination operation at all. This is what makes it non-executable rather than merely false.

"This statement is false" — discrimination cannot resolve because the reference is circular. The content points only to itself with no base case. There is no stable input for discrimination to operate on. {P, ¬P} — discrimination cannot resolve because it is asked to return contradictory verdicts on the same target. The operation does not return "false." It does not return at all. Classical logic's explosion — from a contradiction, anything follows — is the formal system's representation of this: when discrimination collapses on an input, the consequence relation trivializes, which is the formal signature of "no discriminative resolution possible here."

In each case, the expression is syntactically well-formed — it passes the grammar — but discrimination cannot return a verdict because the expression negates a condition required for the discrimination operation to resolve.
