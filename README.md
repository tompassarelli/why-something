...
*A note on method: this paper operates pre-formally. It argues from the conditions under which evaluation, structure, and distinction are possible at all — beneath any particular formal semantics. Formal logic and model theory are discussed where they intersect the argument, but the framework is not built from them and should not be translated into them. The primitive is consistency — the discrimination operation — not truth-conditions, satisfaction relations, or consequence relations. Those are downstream representations. The claim is not that formal semantics are illegitimate, but that any formal semantics presupposes an executable discrimination floor. The argument targets what must already be in place for any such representation to function.*
...
5. An expression is non-executable if under its intended reading discrimination cannot reach a determinate exit condition on it, either because it cancels a condition constitutive of discrimination's applicability (substrate-cancellation) or because it fails to supply a verdict-stabilizing target (anchor-failure). It parses. Discrimination cannot return a verdict.

6. "Nothing exists" and "this statement is false" are two instances of one pattern: expressible, not executable. Both block discrimination from reaching a verdict — the first by canceling the discriminative substrate, the second by failing to provide a verdict-stabilizing target.
...
"Expressible but not executable" is used throughout this paper in a precise sense: the string can be formed and treated as content, but when discrimination is applied to it, no verdict is available. The string parses. It is valid content. But the operation it describes does not resolve into a judgment. It is not that the operation returns an unfamiliar or third kind of result. It returns no verdict. The evaluation does not complete. The failure is not a third verdict. It is the absence of a verdict.
...
The criterion is general: an expression e is non-executable if under its intended reading, discrimination cannot reach a determinate exit condition on e because e either (a) cancels a condition constitutive of discrimination's applicability to that content (substrate-cancellation), or (b) fails to supply a verdict-stabilizing target for discrimination to operate on (anchor-failure). "Constitutive" means: removing the condition does not make the expression false — it removes the possibility of a verdict on that content at all. This is a criterion about what the expression requires, not about who is checking. The question is not whether the act of checking e requires something e denies — almost any globally quantified denial would meet that test, since someone is always doing the checking. The question is whether discrimination can return a verdict on e without presupposing what e cancels or without a target that bottoms out.

"There are no tables" — discrimination resolves. The expression picks out a discriminable pattern (tables) without attacking discrimination itself. The check succeeds or fails by ordinary discrimination. Evaluable. "There are no living things anywhere" — discrimination resolves for the same reason. It targets a discriminable pattern (life) and does not cancel the discriminative substrate. If there is life, discrimination returns a negative verdict; if there is not, it returns a positive one. The act of checking requires a living thing, but discrimination on the expression does not. Executable.

"Nothing exists" on the absolute reading is different. Discrimination cannot resolve because the expression cancels the conditions under which discrimination operates: structure, distinction, anything to discriminate. There is nothing for discrimination to get purchase on. The expression does not return a negative verdict. It fails to reach the discrimination operation at all. This is what makes it non-executable rather than merely false.

"This statement is false" — discrimination cannot resolve because the sentence provides no verdict-stabilizing target. It makes the verdict depend only on itself with no independent anchor, so the operation cannot reach a determinate exit condition.

{P, ¬P} is a related but distinct case. Read as a single compound claim — "both P and not-P" — it is executable: discrimination returns a negative verdict. The conjunction fails to hold. Read as a constraint set — "require P to hold and require ¬P to hold simultaneously" — it is an inconsistent specification that trivializes inference under classical rules (explosion). But trivialization of inference is a different failure mode from non-executability. Discrimination can return a verdict on the conjunction (it fails). What it cannot do is use the set as a consistent basis for further inference. That problem — inconsistent premise sets and what to do with them — is addressed separately in Passarelli (2026). Here, the relevant non-executable pattern has two instances: "nothing exists" and "this statement is false." Both block discrimination from reaching a verdict — the first by canceling the discriminative substrate, the second by failing to supply a verdict-stabilizing target. {P, ¬P} denies a condition required for inference to proceed, which is a downstream phenomenon.

In both non-executable cases, the expression is syntactically well-formed — it passes the grammar — but discrimination cannot return a verdict. "Nothing exists" cancels the discriminative substrate. "This statement is false" fails to supply a verdict-stabilizing target. Both block discrimination from reaching a determinate exit condition.

"Blocks" is precise throughout: under the intended reading, e either cancels a condition constitutive of discrimination's applicability to e, or fails to provide a target that allows discrimination to stabilize a verdict on e. "There are no tables" does neither. "Nothing exists" on the absolute reading cancels the substrate. "This statement is false" fails to provide a verdict-stabilizing target. The criterion does not overgenerate because it operates at the level of discriminative resolution, not at the level of who happens to be checking.

For globally quantified denials — "nothing," "nowhere," "no structure whatsoever" — the relevant scope is necessarily global: any admissible local context in which the expression could be evaluated would itself change the content from the intended absolute denial to a restricted relative one. The scope constraint is a disambiguation rule: evaluating a global denial under a local semantics evaluates a different proposition, not the intended absolute claim. There is no "other evaluation context" to retreat to, because the claim denies all of them.

Non-executable is not undecidable. An undecidable problem is one no algorithm can settle in general, but each instance still has a determinate answer that evaluation could in principle reach. A non-executable expression is one where evaluation cannot get purchase at all without importing what the expression denies. The limitation is not computational. It is structural.
...
The identity required here is minimal: invariance of reference under evaluation. Whatever is being evaluated must remain the same thing throughout the evaluation. This is not a metaphysical claim about the deep nature of objects. It is the condition under which evaluation is evaluation of something rather than nothing determinate. Token identity (this symbol is the same symbol) and object identity (this thing is this thing) are both instances of the same requirement at different configurations — and anything describable at all must satisfy it, because describing requires that what is described holds still long enough to be described.
...
If reality could be inconsistent somewhere, that somewhere would need to be a location — in a structure — distinguished from other locations — by identity and distinction — all of which are consistency operating. Any purported region of inconsistency that remains a region — re-identifiable, delimitable, describable — already satisfies the consistency conditions required to be a region. The only inconsistency that escapes that is not a special region but the collapse of regionhood itself, which is the same null target as absolute nothing. "Inconsistent reality" does not mean "reality with wild parts beyond our ken." It means no identity, no distinction, no structure, no location, no persistence. That is not a region of reality the evaluator cannot reach. It is the same "nothing" the paper already dissolves. "Reality without consistency" meets the non-executable criterion directly: the expression cancels a condition (consistency) constitutive of discrimination's applicability. Epistemology and ontology are not separate domains bridged by an inference. They are different descriptions of the same thing — consistency at different configurations. The gap between them is generated by assuming a dualism the framework shows to be incoherent.

True and false are evaluation outputs, not symbols that can be extended or combined. Evaluation has two verdicts when it returns a verdict at all. A process that does not terminate has not produced a verdict. It has not returned. Treating non-termination as a truth value is like treating silence as a type of statement. Non-bivalent logics, paraconsistent systems, and dialetheism introduce notation that permits contradiction-shaped strings at the object level. But the meta-level evaluation that certifies those systems — defining their truth values, proving their properties, checking that the notation behaves as intended — is itself bivalent. It presupposes the consistency floor it claims to move beyond. These systems do not extend evaluation. They introduce bookkeeping whose correctness is certified by the same two-verdict evaluation they purport to surpass. The question of whether they have any legitimate use case at all is addressed separately in Passarelli (2026), which argues they do not.

A statement that cannot be evaluated, such as "this statement is false," is not a paradox. It is an expression that does not execute. The string exists. It is valid content. But the operation it requests — evaluating a pointer that refers only to itself with no base case — does not terminate. There is no proposition to assess. The elaborate formal machinery built to handle it — Tarski hierarchies, Kripke fixed points, dialethic truth — is machinery designed to process an input that should have been rejected at validation. A fixed-point theory can classify the sentence as "ungrounded" — that is a meta-level verdict about the evaluation attempt, not a truth verdict for the sentence. The classification confirms the paper's claim: the sentence does not resolve into a truth value. Calling it "ungrounded" is agreeing that it does not execute while giving the non-execution a label.
...
The "nothing" side. Nothing is expressible. The string exists. Consistency permits it as content. But "nothing" is not executable. It is an operation that targets the host container — the removal of all structure, including the evaluator processing the removal. The operation does not terminate. Its non-completion is a manifestation of the deeper point: the specification cancels the discriminative conditions needed for any verdict. It is not that nothing is excluded from the space of possibilities by a rule. It is that the specification "nothing" cannot serve as input to discrimination at all. Content can be deallocated. The evaluator cannot deallocate itself. Things end. Consistency does not.
...
This paper's principle: a possibility must have specification conditions that can be stated without self-undermining — not by us, not by any particular formal system, but in principle. A "possibility" whose specification conditions cannot be coherently stated is not a possibility. It is the word "possible" doing no work.
...
Pending is not a status of the evaluation. It is a non-return. Evaluation has two verdicts when defined. A function that has not returned has not returned a third verdict. It has not returned. The distinction is between verdicts and non-verdicts, not between three kinds of verdict. A process that loops forever does not produce a result that is "neither true nor false." It produces no result. Confusing "no output" with "a third output" is a category error — the same category error this paper identifies throughout: treating the lack of a thing as a special kind of the thing.
